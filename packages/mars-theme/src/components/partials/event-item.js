...
const EventItem = ({ event }) => {
  ...
  useEffect(() => {
    if (event._links['wp:featuredmedia']) {
      axios.get(event._links['wp:featuredmedia'][0].href)
        .then(response => {
  ...
  useEffect(() => {
    if (event._links['wp:term']) {
      const categoriesLink = event._links['wp:term'].find(link => link.taxonomy === 'events-category');
      if (categoriesLink) {
  ...
  return (
    <div className="events__item" css={styles}>
      <a href={event.link}>
        ...
        <h3 className="events__item-title title-5">
